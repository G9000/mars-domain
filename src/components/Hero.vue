...
const minting = async() => {
    ...
    if (domain.name.length > 3) {
        ...
        if (mintRes && mintRes.code === 200) {
            ...
            isError.value = true
            errorMsg.value = 'It’s look like we have encountered a problem. Apologize for the inconvenience. Must be the solar wind.'
        }        
    } else {
        isError.value = true
...
<template>
    <!-- <div class="min-h-screen h-full flex items-center justify-center bg-top bg-contain bg-no-repeat" style="background-image: url('/src/assets/mars.png')"></div> -->
    <div class="h-screen flex items-center justify-center">
        <div class="flex flex-col items-center gap-y-6">
            <h1 class="text-white font-bold tracking-wide text-center">Building The Martian Digital Landmark</h1>

            <div v-if="!isError" class="mt-4">
                <div v-if="!isMinting" class="flex flex-col gap-y-4">
                    <form class="flex flex-col gap-y-4 text-white" autocomplete="off" @submit.prevent="minting">
                    <div class="flex items-center w-full h-[50px]">
                        <div class="p-4 h-full w-full bg-black">
                            <label for="domainName" class="hidden-visually">Domain name: </label>
                            <input
                                type="text"
                        ...
                        <div class="flex items-center text-3xl h-full font-bold px-4 bg-brandOrange text-[#88390D]">
                            .mars
                        </div>
                    </div>
                        <div class="p-4 h-full w-full bg-black">
                            <label for="domainDetails" class="hidden-visually">Domain details: </label>
                            <textarea
                                type="text"
                    ...
                    <span class="text-gray-300 text-center">{{ helperText.first }}<a :href="helperText.href" target="_blank" class="font-semibold text-white">{{ helperText.second }}</a></span>
                </div>

                <div v-else class="mt-6 bg-brandOrange py-2 px-6 text-white font-bold tracking-wider text-xl md:text-2xl">
                    Minting . . .
                </div>
            </div>

            <div v-else class="flex flex-col items-center mt-6 gap-x-4 py-6 bg-brandOrange max-w-[460px]">
                <div class="px-6 flex items-center h-full text-white tracking-wider text-base md:text-md text-center">
                    <span>{{ errorMsg }}</span>
                </div>
                <button class="bg-brandOrange h-full px-6 text-white mt-4 md:mt-8 underline" @click="resetForm">Retry</button>
            </div>
